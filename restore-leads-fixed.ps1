...
try {
    ...
    $missingLead1 = [PSCustomObject]@{
        id = "$(Get-Date -UFormat %s)000-missing1"
    ...
    $missingLead2 = [PSCustomObject]@{
        id = "$(Get-Date -UFormat %s)000-missing2"
    ...
    $jsonPayload = $restoreData | ConvertTo-Json -Depth 10
    ...
    if ($restoreResponse.StatusCode -eq 200) {
        ...
        Write-Host "   1. Restored Lead 1 (Under review)" -ForegroundColor Cyan
        Write-Host "   2. Restored Lead 2 (Won)" -ForegroundColor Cyan
    } else {
